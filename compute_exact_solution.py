...
for k in range(500):
    ...
    u_exact, u_exact_60 = train_model.compute_exact_end(Buckley_Leverett, problem_ex, 60, 36, just_one_time_step=False, trainable=False)
    u_exact = u_exact.detach().numpy()
    u_exact_60 = u_exact_60.detach().numpy()
    np.save("Exact_Solutions/u_exact_{}".format(k),u_exact)
    np.save("Exact_Solutions/u_exact_60_{}".format(k), u_exact_60)
    parameters.append([width, height])

#pd.DataFrame(parameters)
df=pd.DataFrame(parameters, columns=["width", "height"])
df.to_csv("Exact_Solutions/parameters.csv",index=False)
